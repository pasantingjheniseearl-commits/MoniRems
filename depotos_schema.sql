...
begin
  ...
  values (
    ...
    new.email,
    coalesce((new.raw_user_meta_data->>'role')::public.app_role, 'Stock Clerk')
  );
  return new;
end;
$$;
...
-- their profiles row (name, username, role) automatically.
--
-- If your Supabase project has "Confirm email" turned on
-- (Authentication → Providers → Email), new users must click the
-- confirmation link before they can sign in. For an internal
-- tool where you don't want that step, turn it off there.
-- =========================================================
